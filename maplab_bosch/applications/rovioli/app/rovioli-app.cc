...
DEFINE_bool(
    ...
    "Use wheel odometry to give velocity updates to the filter state.");
DEFINE_string(
    wheel_parameters, "wheel.yaml",
    "Path to the wheel odometry configuration yaml for MAPLAB.");
DEFINE_string(
...
int main(int argc, char** argv) {
  ...
  // Read wheel odometry config
  vi_map::Relative6DoFPose::UniquePtr wheel_sensor(nullptr);
  if (FLAGS_use_wheel_odometry) {
    wheel_sensor = vi_map::createFromYaml<vi_map::Relative6DoFPose>(
        FLAGS_wheel_parameters);

    CHECK(wheel_sensor)
        << "Could not load wheel odometry parameters for MAPLAB from: \'"
        << FLAGS_wheel_parameters << "\'";
  }
  ...
  rovioli::RovioliNode rovio_localization_node(
      ...
      std::move(wheel_sensor), save_map_folder, localization_map.get(),
      flow.get());
