...
ANY_RBDL_DLLAPI int rbdl_get_api_version() {
	static int compile_version = ANY_RBDL_API_VERSION;
	return compile_version;
}

ANY_RBDL_DLLAPI void rbdl_check_api_version(int version) {
	...
	if (version_major != compile_major) {
		std::cerr << "Error: trying to link against an incompatible ANY_RBDL library." << std::endl;
		...
		std::cout << "Warning: ANY_RBDL library is of version " << compile_version_string.str() << " but rbdl_config.h is from version " << link_version_string.str() << std::endl;
	}
}

ANY_RBDL_DLLAPI void rbdl_print_version() {
	int compile_version = rbdl_get_api_version();
	...
	std::cout << "ANY_RBDL version:" << std::endl
		<< "  API version  : " << compile_version_string.str() << std::endl;

	if (std::string("unknown") != ANY_RBDL_BUILD_REVISION) {
		std::cout << "  revision     : " << ANY_RBDL_BUILD_REVISION
		<< " (branch: " << ANY_RBDL_BUILD_BRANCH << ")" << std::endl
		<< "  build type   : " << ANY_RBDL_BUILD_TYPE << std::endl;
	}

#ifdef ANY_RBDL_ENABLE_LOGGING
...
#ifdef ANY_RBDL_BUILD_ADDON_LUAMODEL
	std::cout << "  LuaModel     : on" << std::endl;
#else
	std::cout << "  LuaModel     : off" << std::endl;
#endif
#ifdef ANY_RBDL_BUILD_ADDON_URDFREADER
	std::cout << "  URDFReader   : on" << std::endl;
#else
	std::cout << "  URDFReader   : off" << std::endl;
#endif

	std::string build_revision (ANY_RBDL_BUILD_REVISION);
	if (build_revision == "unknown") {
		std::cout << std::endl << "Version information incomplete: to enable version information re-build" << std::endl << "library from valid repository and enable ANY_RBDL_STORE_VERSION." << std::endl;
	}
}
