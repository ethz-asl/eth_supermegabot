 ...
 */
enum ANY_RBDL_DLLAPI LinearSolver {
...
extern ANY_RBDL_DLLAPI Vector3d Vector3dZero;
extern ANY_RBDL_DLLAPI Matrix3d Matrix3dIdentity;
extern ANY_RBDL_DLLAPI Matrix3d Matrix3dZero;

extern ANY_RBDL_DLLAPI SpatialVector SpatialVectorZero;
extern ANY_RBDL_DLLAPI SpatialMatrix SpatialMatrixIdentity;
extern ANY_RBDL_DLLAPI SpatialMatrix SpatialMatrixZero;
...
// \todo write test
ANY_RBDL_DLLAPI void SpatialMatrixSetSubmatrix(SpatialMatrix &dest, unsigned int row, unsigned int col, const Matrix3d &matrix);

ANY_RBDL_DLLAPI bool SpatialMatrixCompareEpsilon (const SpatialMatrix &matrix_a,
						  const SpatialMatrix &matrix_b, double epsilon);
ANY_RBDL_DLLAPI bool SpatialVectorCompareEpsilon (const SpatialVector &vector_a,
						  const SpatialVector &vector_b, double epsilon);

/** \brief Translates the inertia matrix to a new center. */
ANY_RBDL_DLLAPI Matrix3d parallel_axis (const Matrix3d &inertia, double mass, const Vector3d &com);
 ...
 */
ANY_RBDL_DLLAPI SpatialMatrix Xtrans_mat (const Vector3d &displacement);
 ...
 */
ANY_RBDL_DLLAPI SpatialMatrix Xrotz_mat (const double &zrot);
 ...
 */
ANY_RBDL_DLLAPI SpatialMatrix Xroty_mat (const double &yrot);
 ...
 */
ANY_RBDL_DLLAPI SpatialMatrix Xrotx_mat (const double &xrot);
 ...
 * by ZYX-Euler angles.
 */
ANY_RBDL_DLLAPI SpatialMatrix XtransRotZYXEuler (const Vector3d &displacement, const Vector3d &zyx_euler);

ANY_RBDL_DLLAPI inline Matrix3d rotx (const double &xrot) {
	double s, c;
	s = sin (xrot);
	c = cos (xrot);
	return Matrix3d (
...
ANY_RBDL_DLLAPI inline Matrix3d roty (const double &yrot) {
	double s, c;
	s = sin (yrot);
	c = cos (yrot);
	return Matrix3d (
...
ANY_RBDL_DLLAPI inline Matrix3d rotz (const double &zrot) {
	double s, c;
	s = sin (zrot);
	c = cos (zrot);
	return Matrix3d (
...
ANY_RBDL_DLLAPI inline Matrix3d rotxdot (const double &x, const double &xdot) {
	double s, c;
	s = sin (x);
	c = cos (x);
	return Matrix3d (
...
ANY_RBDL_DLLAPI inline Matrix3d rotydot (const double &y, const double &ydot) {
	double s, c;
	s = sin (y);
	c = cos (y);
	return Matrix3d (
...
ANY_RBDL_DLLAPI inline Matrix3d rotzdot (const double &z, const double &zdot) {
	double s, c;
	s = sin (z);
	c = cos (z);
	return Matrix3d (
...
ANY_RBDL_DLLAPI inline Vector3d angular_velocity_from_angle_rates (const Vector3d &zyx_angles, const Vector3d &zyx_angle_rates) {
...
ANY_RBDL_DLLAPI inline Vector3d global_angular_velocity_from_rates (const Vector3d &zyx_angles, const Vector3d &zyx_rates) {
...
ANY_RBDL_DLLAPI inline Vector3d angular_acceleration_from_angle_rates (const Vector3d &zyx_angles, const Vector3d &zyx_angle_rates, const Vector3d &zyx_angle_rates_dot) {
	double sy = sin(zyx_angles[1]);
	double cy = cos(zyx_angles[1]);
	double sx = sin(zyx_angles[2]);
	double cx = cos(zyx_angles[2]);
