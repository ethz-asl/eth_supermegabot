...
struct ANY_RBDL_DLLAPI Joint {
	...
	Joint ( JointType type,
			...
			const std::string &mimicJoint = "",
			const double &mimicMult = 1.0,
			const double &mimicOffset = 0.0):
		mJointAxes (NULL),
		mJointType (type),
		mDoFCount (0),
		q_index (0),
		...
		mMimicMult(mimicMult),
		mMimicOffset(mimicOffset)
	{
	...
	Joint (const Joint &joint) :
		...
		mName(joint.mName),
		mMimicJoint(joint.mMimicJoint),
		mMimicMult(joint.mMimicMult),
		mMimicOffset(joint.mMimicOffset) {
			mJointAxes = new Math::SpatialVector[mDoFCount];
  ...
  /** \brief Checks whether this joint mimics another.
   *
   *  Mimic name as well as nonzero values for either offset or mult has to be set.
   */
	bool is_mimicing() const{
	  return !mMimicJoint.empty();
	}
