...
RovioFlow::RovioFlow(
  ...
  use_wheel_odometry_ = false;
  T_I_O_.setIdentity();
}

RovioFlow::~RovioFlow() {}

void RovioFlow::includeWheelOdometry(aslam::Transformation T_I_O) {
  use_wheel_odometry_ = true;
  T_I_O_ = T_I_O;
}
...
void RovioFlow::attachToMessageFlow(message_flow::MessageFlow* flow) {
  ...
  // Input Odometry.
  if (use_wheel_odometry_) {
    flow->registerSubscriber<message_flow_topics::ODOMETRY_MEASUREMENTS>(
        kSubscriberNodeName, rovio_subscriber_options,
        [this](const vio::OdometryMeasurement::ConstPtr& odometry) {
          Eigen::Vector3d t_I_O = T_I_O_.getPosition();
          Eigen::Matrix3d R_I_O = T_I_O_.getRotationMatrix();

          Eigen::Vector3d v_I_O =
              R_I_O * odometry->velocity_linear -
              (R_I_O * odometry->velocity_angular).cross(t_I_O);
          ...
          const bool measurement_accepted =
              this->rovio_interface_->processVelocityUpdate(
                  v_I_O, rovio_timestamp_sec);
          LOG_IF(
              WARNING,
              !measurement_accepted && rovio_interface_->isInitialized())
