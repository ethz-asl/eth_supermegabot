...
RovioliNode::RovioliNode(
    ...
    vi_map::Relative6DoFPose::UniquePtr wheel_sensor,
  ...
  vio_common::RosTopicSettings topic_settings(
      *camera_system, *maplab_imu_sensor);
  if (wheel_sensor != nullptr) {
    topic_settings.absolute_pose_topic = wheel_sensor->getHardwareId();
    CHECK(!topic_settings.absolute_pose_topic.empty());
  }
  datasource_flow_.reset(new DataSourceFlow(topic_settings));
  datasource_flow_->attachToMessageFlow(flow);

  rovio_flow_.reset(
      new RovioFlow(*camera_system, rovio_imu_sigmas, *wheel_sensor));
  if (wheel_sensor != nullptr) {
    rovio_flow_->includeWheelOdometry(wheel_sensor->get_T_R_S());
  }
