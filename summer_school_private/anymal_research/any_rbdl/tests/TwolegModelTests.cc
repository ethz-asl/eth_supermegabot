...
TEST (TwoLegModel,  TestForwardDynamicsContactsDirectFootmodel ) {
...
//	cout << contact_accel_left << endl;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (Vector3d (0., 0., 0.), contact_accel_left, TEST_PREC, TEST_PREC, "");

	delete model;
}

TEST (TwoLegModel,  TestClearContactsInertiaMatrix ) {
	Model* model = new Model;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_lag, QDDot_aba, TEST_PREC, TEST_PREC * QDDot_lag.norm(), "");
