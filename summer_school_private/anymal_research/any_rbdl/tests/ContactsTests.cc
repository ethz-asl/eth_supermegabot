...
struct FixedBase6DoF9DoF : ::testing::Test {
	FixedBase6DoF9DoF () {
		ClearLogOutput();
		model = new Model;
...
//
TEST (Contacts,  TestForwardDynamicsContactsDirectSimple ) {
	Model model;
	model.gravity = Vector3d  (0., -9.81, 0.);
...
TEST (Contacts,  TestForwardDynamicsContactsDirectMoving ) {
	Model model;
	model.gravity = Vector3d  (0., -9.81, 0.);
	Body base_body (1., Vector3d (0., 0., 0.), Vector3d (1., 1., 1.));
	unsigned int base_body_id = model.SetFloatingBaseBody(base_body);
...
//	cout << LogOutput.str() << endl;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (point_accel_lagrangian, point_accel_contacts, TEST_PREC, TEST_PREC, "");
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_lagrangian, QDDot_contacts, TEST_PREC, TEST_PREC, "");
}
...
TEST_F (FixedBase6DoF, ForwardDynamicsContactsSingleContactRotated) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (point_accel_lagrangian, point_accel_contacts_opt, TEST_PREC, TEST_PREC, "");
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_lagrangian, QDDot_contacts_opt, TEST_PREC, TEST_PREC, "");
}
...
//	cout << "Lagrangian contact force " << contact_data_lagrangian[0].force << ", " << contact_data_lagrangian[1].force << ", " << contact_data_lagrangian[2].force << endl;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_lagrangian, QDDot, TEST_PREC, TEST_PREC, "");
}
...
TEST_F (Human36, ForwardDynamicsContactsFixedBody) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (qddot_lagrangian, qddot, TEST_PREC, TEST_PREC * qddot_lagrangian.norm(), "");
...
TEST_F (Human36, ForwardDynamicsContactsImpulses) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (Vector3d(0., 0., 0.), heel_left_velocity, TEST_PREC, TEST_PREC, "");
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (Vector3d(0., 0., 0.), heel_right_velocity, TEST_PREC, TEST_PREC, "");
}
