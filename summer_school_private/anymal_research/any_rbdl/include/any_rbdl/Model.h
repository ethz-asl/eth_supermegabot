...
struct ANY_RBDL_DLLAPI Model {
	...
	/// \brief A vector of all q indexes that are mimiced
	std::unordered_set<unsigned int> mimic_qs;
	...
	/// \brief Id of the root in the bodynamemap
	unsigned int rootId;
	...
	/** \brief Set the id of a the root body by name
	 *
	 * @param rootName name of the root.
	 *
	 */
	void SetRootIdByName(const std::string & rootName) {
		rootId = GetBodyId(rootName.c_str());
	}
	...
	/** Returns a list of movable joint IDs between two bodies.
	 *
	 * Searches through the kinematic tree to determine which joints
	 * lie between two bodies.
	 * The list is in no guaranteed order.
	 * The list is empty if bodies are not in the same branch of the tree.
	 */
	std::list<unsigned int> GetJointIdsBodyToBody(unsigned int bodyA, unsigned int bodyB);
...
// Functions for mimics treatment // TODO write this based on Base class so we do not need two seperate functions
template <unsigned int Rows_, unsigned int Cols_>
void getMimicMatrix (Eigen::Block<Eigen::MatrixXd , Rows_, Cols_>&& outMatrix, const Eigen::Matrix<double, Rows_, Cols_>& inMatrix, const Model& model, unsigned int q_index, std::unordered_set<unsigned int> & proccessed_mimic_joints) {
	// Are there mimic joints at all? - Is the current joint mimiced?
	if(!model.mimic_qs.size() || model.mimic_qs.find(q_index) == model.mimic_qs.end()) {
		outMatrix = inMatrix;
		return;
	}
	// first time mimic joint is called?
	else if(proccessed_mimic_joints.find(q_index) == proccessed_mimic_joints.end() ) {
		outMatrix = inMatrix;
		proccessed_mimic_joints.insert(q_index);
	}
	else {
		outMatrix += inMatrix;
	}
...
template <unsigned int Rows_, unsigned int Cols_>
void getMimicMatrix (Eigen::Block<Eigen::MatrixXd , Rows_, Cols_>&& outMatrix,
					 const Eigen::Matrix<double, Rows_, Cols_>& inMatrix,
					 const Model& model,
					 unsigned int qi_index,
					 unsigned int qj_index,
					 std::set<std::pair<unsigned int, unsigned int>> & proccessed_mimic_joints) {
	// Are there mimic joints at all? - Is the current joint mimiced?
	if(!model.mimic_qs.size() || ( model.mimic_qs.find(qi_index) == model.mimic_qs.end() && model.mimic_qs.find(qj_index) == model.mimic_qs.end()) ) {
		outMatrix = inMatrix;
		return;
	}
	// first time mimic joint is called?
	else {
		auto it = std::find_if(proccessed_mimic_joints.begin(), proccessed_mimic_joints.end(),
					 [qi_index, qj_index](const std::pair<unsigned int, unsigned int> & entry){
					   return (qi_index == entry.first) && (qj_index == entry.second); } );
		if(it == proccessed_mimic_joints.end() ) {
			outMatrix = inMatrix;
			proccessed_mimic_joints.insert(std::make_pair(qi_index, qj_index));
		}
		else {
			outMatrix += inMatrix;
		}
...
template <unsigned int Rows_>
void getMimicVector (Eigen::Block<Eigen::VectorXd , Rows_, 1>&& outMatrix, const Eigen::Matrix<double, Rows_, 1>& inMatrix, const Model& model, unsigned int q_index, std::unordered_set<unsigned int> & proccessed_mimic_joints) {
	// Are there mimic joints at all? - Is the current joint mimiced?
	if(!model.mimic_qs.size() || model.mimic_qs.find(q_index) == model.mimic_qs.end()) {
		outMatrix = inMatrix;
		return;
