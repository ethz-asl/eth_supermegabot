...
#include "any_rbdl/addons/urdfreader/urdfreader.h"
...
ANY_RBDL_DLLAPI bool URDFReadFromFile (const char* filename, Model* model, bool verbose) {
...
ANY_RBDL_DLLAPI bool URDFReadFromString (const char* model_xml_string, Model* model, bool verbose) {
  assert (model);
...
bool construct_model (Model* rbdl_model, ModelPtr urdf_model,
  ...
  // attempt to find the root inertial
  urdf::LinkConstSharedPtr root_inertial = root;
  URDFLinkMap::const_iterator root_inertial_it = link_map.find(root->name+"_inertia");
  if (root_inertial_it != link_map.end()) {
	root_inertial = root_inertial_it->second;
  }

  if (root_inertial->inertial) {
	root_inertial_mass = root_inertial->inertial->mass;

	root_inertial_position.set (
		root_inertial->inertial->origin.position.x,
		root_inertial->inertial->origin.position.y,
		root_inertial->inertial->origin.position.z);

	root_inertial_inertia(0,0) = root_inertial->inertial->ixx;
	root_inertial_inertia(0,1) = root_inertial->inertial->ixy;
	root_inertial_inertia(0,2) = root_inertial->inertial->ixz;

	root_inertial_inertia(1,0) = root_inertial->inertial->ixy;
	root_inertial_inertia(1,1) = root_inertial->inertial->iyy;
	root_inertial_inertia(1,2) = root_inertial->inertial->iyz;

	root_inertial_inertia(2,0) = root_inertial->inertial->ixz;
	root_inertial_inertia(2,1) = root_inertial->inertial->iyz;
	root_inertial_inertia(2,2) = root_inertial->inertial->izz;

	root_inertial->inertial->origin.rotation.getRPY (root_inertial_rpy[0], root_inertial_rpy[1], root_inertial_rpy[2]);
  ...
  typedef std::map<std::string, std::list<std::string>::iterator> MovableJointMap;

  MovableJointMap movable_joints;
  for (unsigned int i = 0; i < movable_joints_in_order.size(); ++i) {
	joint_names.push_back(movable_joints_in_order[i]);
	movable_joints.insert(std::make_pair(movable_joints_in_order[i],
						  --joint_names.end()));
  }
  ...
  list<string>::iterator joint_it = joint_names.begin();
  ...
  while (link_stack.size() > 0) {
	...
	if (joint_idx < cur_link->child_joints.size()) {
	  ...
	  MovableJointMap::iterator movable_joint_it = movable_joints.find(cur_joint->name);

	  if (movable_joint_it != movable_joints.end()) {
		joint_it = movable_joint_it->second;
	  }
	  else {
		joint_it = joint_names.insert(++joint_it, cur_joint->name);
	  }
	} else {
	  link_stack.pop();
	  joint_index_stack.pop();
  ...
  for (list<string>::const_iterator it = joint_names.begin(); it != joint_names.end(); it++) {
	JointPtr urdf_joint = joint_map[*it];
	LinkPtr urdf_parent = link_map[urdf_joint->parent_link_name];
	LinkPtr urdf_child = link_map[urdf_joint->child_link_name];

	// ignore the root inertial joint
	if (urdf_child->name == root_inertial->name)
	  continue;

	// determine where to add the current joint and child body
	unsigned int rbdl_parent_id = 0;

	// join bodies with root body instead of the roots inertial
	if (urdf_parent->name != root_inertial->name)
	  rbdl_parent_id = rbdl_model->GetBodyId(urdf_parent->name.c_str());
	else
	  rbdl_parent_id = rbdl_model->GetBodyId(root->name.c_str());
...
ANY_RBDL_DLLAPI bool URDFReadFromFile (const char* filename, Model* model,
  ...
  return URDFReadFromString (model_xml_string.c_str(), model, movable_joints_in_order, verbose);
}
...
ANY_RBDL_DLLAPI bool URDFReadFromString (const char* model_xml_string, Model* model,
  ...
  if (!construct_model (model, urdf_model, movable_joints_in_order, verbose)) {
	cerr << "Error constructing model from urdf file." << endl;
	return false;
  }
