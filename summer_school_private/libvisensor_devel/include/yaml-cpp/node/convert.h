...
namespace YAML
{
    namespace conversion {
        DSO_EXPORT inline bool IsInfinity(const std::string& input) {
            return input == ".inf" || input == ".Inf" || input == ".INF" || input == "+.inf" || input == "+.Inf" || input == "+.INF";
        }
        
        DSO_EXPORT inline bool IsNegativeInfinity(const std::string& input) {
            return input == "-.inf" || input == "-.Inf" || input == "-.INF";
        }
        
        DSO_EXPORT inline bool IsNaN(const std::string& input) {
	...
	// std::string
	template<>
	struct DSO_EXPORT convert<std::string> {
	...
	template<>
	struct DSO_EXPORT convert<const char *> {
	...
	struct DSO_EXPORT convert<const char[N]> {
  ...
  template<>
  struct DSO_EXPORT convert<_Null> {
...
#define YAML_DEFINE_CONVERT_STREAMABLE(type, negative_op)\
	template<>\
	struct DSO_EXPORT convert<type> {\
...
#undef YAML_DEFINE_CONVERT_STREAMABLE
	...
	// bool
	template<>
	struct DSO_EXPORT convert<bool> {
	...
	struct DSO_EXPORT convert<std::map<K, V> > {
...
//workaround for GCC 3:
	...
	// std::vector
	template<typename T>
	struct DSO_EXPORT convert<std::vector<T> > {
...
//workaround for GCC 3:
	...
	// std::list
	template<typename T>
	struct DSO_EXPORT convert<std::list<T> > {
...
//workaround for GCC 3:
	...
	struct DSO_EXPORT convert<std::pair<T, U> > {
...
//workaround for GCC 3:
  ...
  // binary
  template<>
  struct DSO_EXPORT convert<Binary> {
